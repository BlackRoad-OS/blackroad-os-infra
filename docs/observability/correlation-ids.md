...
When a user request touches multiple services (e.g., web → api → database), correlation IDs let you:
...
┌──────────────────────────────────────────────────────────────────┐
│ User Request                                                      │
│ X-Correlation-ID: corr_user123session456                         │
└────────────────────────────────────┬─────────────────────────────┘
                                     │
                                     ▼
┌──────────────────────────────────────────────────────────────────┐
...
│ X-Correlation-ID: corr_user123session456                         │
│ X-Request-ID: req_web001                                          │
│                                                                   │
│ LOG: {"requestId":"req_web001","correlationId":"corr_user..."}   │
└────────────────────────────────────┬─────────────────────────────┘
                                     │
                                     ▼
┌──────────────────────────────────────────────────────────────────┐
│ blackroad-os-api (Backend)                                        │
│ X-Correlation-ID: corr_user123session456 (propagated)            │
│ X-Request-ID: req_api001 (new for this service)                   │
│ X-Parent-Request-ID: req_web001 (upstream request)                │
│                                                                   │
│ LOG: {"requestId":"req_api001","correlationId":"corr_user..."}   │
└──────────────────────────────────────────────────────────────────┘
```
...
- ❌ Generating new correlation ID at each service
- ❌ Not propagating IDs to downstream calls
- ❌ Logging without request context
- ❌ Using different ID formats per service
